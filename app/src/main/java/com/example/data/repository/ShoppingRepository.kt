...
class ShoppingRepository(
    ...
    suspend fun completePurchaseAndClear(listId: String) = withContext(Dispatchers.IO) {
        ...
        val historyItems = purchased.map { item ->
            com.example.data.local.PurchaseHistoryItemEntity(
                id = UUID.randomUUID().toString(),
                historyId = historyId,
                name = item.name,
        ...
        purchaseHistoryDao.insertItems(historyItems)
        ...
        return try {
            client.newCall(request).execute().use { response ->
                if (response.isSuccessful && response.body != null) {
                    ...
                    val adapter = SupabaseClient.moshi.adapter(com.example.data.remote.AuthResponse::class.java)
                    val authResponse = try {
                        adapter.fromJson(responseBody)
                    } catch (e: Exception) {
                        null
                    }
                    if (authResponse != null && !authResponse.accessToken.isNullOrBlank() && !authResponse.refreshToken.isNullOrBlank()) {
                        session.updateTokens(authResponse.accessToken, authResponse.refreshToken)
                        true
                    } else {
                        false
                    }
